...
<head>
	...
	@if($slug)
		...
		<meta property="og:image" content="{{ $ogImage ?? URL::og($ogTitle ?? $title, url()->current()) }}" />
		...
		<meta name="twitter:image" content="{{ $ogImage ?? URL::og($ogTitle ?? $title, url()->current()) }}" />
	@endif
</head>
		...
		<header class="bg-gray-900 text-white">
			<div class="container mx-auto p-4 flex items-center">
				...
				<div class="ml-auto flex gap-4">
					<a class="no-underline opacity-75 hover:opacity-100"
					   href="https://bsky.app/profile/cmorrell.com"
					   target="_blank"
					   rel="me"
					   title="Chris Morrell on Bluesky"
					>
						<svg class="h-6 w-6 fill-current" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512">
